...
-- 数据源: 股票财经平台(东方财富、雪球、同花顺、新浪财经、金融界、和讯)
...
CREATE TABLE `daily_news` (
    ...
    `source_platform` varchar(32) NOT NULL COMMENT '新闻源平台(eastmoney|xueqiu|tonghuashun|sina_finance|jinrongjie|hexun等)',
...
CREATE TABLE `daily_topics` (
    ...
    KEY `idx_topic_date_status` (`extract_date`, `processing_status`)
...
CREATE TABLE `crawling_tasks` (
    ...
    `platform` varchar(32) NOT NULL COMMENT '目标平台(eastmoney|xueqiu|tonghuashun|sina_finance|jinrongjie|hexun)',
    ...
    KEY `idx_task_topic_platform` (`topic_id`, `platform`, `task_status`),
    FOREIGN KEY (`topic_id`) REFERENCES `daily_topics`(`topic_id`) ON DELETE CASCADE
...
-- 为股票平台现有表添加话题关联字段,支持MindSpider功能
-- 注意:这些字段是可选的,不影响MediaCrawler原有功能

-- 为东方财富帖子表添加话题关联字段
ALTER TABLE `eastmoney_post` 
...
-- 为雪球帖子表添加话题关联字段
ALTER TABLE `xueqiu_post` 
ADD COLUMN `topic_id` varchar(64) DEFAULT NULL COMMENT '关联的话题ID',
ADD COLUMN `crawling_task_id` varchar(64) DEFAULT NULL COMMENT '关联的爬取任务ID';

-- 为同花顺帖子表添加话题关联字段
ALTER TABLE `tonghuashun_post` 
ADD COLUMN `topic_id` varchar(64) DEFAULT NULL COMMENT '关联的话题ID',
ADD COLUMN `crawling_task_id` varchar(64) DEFAULT NULL COMMENT '关联的爬取任务ID';

-- 为新浪财经帖子表添加话题关联字段
ALTER TABLE `sina_finance_post` 
ADD COLUMN `topic_id` varchar(64) DEFAULT NULL COMMENT '关联的话题ID',
ADD COLUMN `crawling_task_id` varchar(64) DEFAULT NULL COMMENT '关联的爬取任务ID';

-- 为金融界帖子表添加话题关联字段
ALTER TABLE `jinrongjie_post` 
ADD COLUMN `topic_id` varchar(64) DEFAULT NULL COMMENT '关联的话题ID',
ADD COLUMN `crawling_task_id` varchar(64) DEFAULT NULL COMMENT '关联的爬取任务ID';

-- 为和讯帖子表添加话题关联字段
ALTER TABLE `hexun_post` 
ADD COLUMN `topic_id` varchar(64) DEFAULT NULL COMMENT '关联的话题ID',
ADD COLUMN `crawling_task_id` varchar(64) DEFAULT NULL COMMENT '关联的爬取任务ID';
